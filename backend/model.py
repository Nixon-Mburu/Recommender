...
def load_and_preprocess_data(csv_path):
    try:
        ...
        if 'rating' not in df.columns:
            raise ValueError("Required column 'rating' not found in the dataset")
    ...
    X = df.drop(['rating', 'title', 'movie_id'] if all(col in df.columns for col in ['rating', 'title', 'movie_id']) 
                else df.columns.difference(['rating']), axis=1)
    y = df['rating'].values
